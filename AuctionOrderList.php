...
if($my_member_id){
$numresults=mysqli_query($link,"select id from auctionpurchase");
...
<td width="710"> &nbsp;<B>경매(총등록수 : <?php echo number_format($numrows); ?>건)</B>
</td>
</tr>
...
$num=1;
$numresult=mysqli_query($link,"select * from auctionpurchase order by id desc limit $fromnum,$limit50");
...
$query=mysqli_query($link,"select * from goods where id='$row[goodsid]' order by id desc limit $fromnum,$limit50");
...
<br>즉시구매가 : <?php echo number_format($row['fixprice']); ?>
<br><font color=red>경매시작가 : <?php echo number_format($row['stprice']); ?></font>
<br><b>최고가 : <font color="#DF5B0B">
<?php echo number_format($row['price']); ?>원</b></font>
<br>등록수량 : <?php echo number_format($row['recordquantity']); ?>개
<br>현재구매수량 : <?php echo number_format($row['orderquantity']); ?>개
<br>마감일자 : 
<?php 
$ordertime=$row['enddate'];
...
<td>
<form name="Form1" method="post" action=auctionOrderWrite.php?id=<?php echo $row['id']?>&goodsid=<?php echo $rowbody['id']?>> 
금액 : <input type="text" name="auctionprice" value="" size="12">원
<span style=font-size:5pt;>&nbsp;</span><br> 
<input type="submit" name=submit value="[경매 신청하기]">
</form>
