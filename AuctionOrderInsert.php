...
if(!isset($_REQUEST['auctionprice'])){$auctionprice  =1;} else {$auctionprice  = $_REQUEST['auctionprice'];}
...
if($auctionprice > $row['price']){
$update = "update auctionpurchase set price='$auctionprice' where id='$id' ";
$result=mysqli_query($link,$update);
}
$soquantity=$row['orderquantity']+1;
$update2 = "update auctionpurchase set orderquantity='$soquantity' where id='$id' ";
$result2=mysqli_query($link,$update2);

$querymyorder=mysqli_num_rows(mysqli_query($link,"select id from auctionorder where ordernum='$auctionordernum' "));
if($querymyorder < 1){ // 사용한 장바구니 체크
$insert = "insert into auctionorder values ".
	"('','$auctionordernum','$my_member_id','$ordername','$row[id]','$auctionprice',".
...
unset($_SESSION['auctionordernum']);
mysqli_close($link);
echo ("<meta http-equiv='Refresh' content='0; URL=MyPage.php'>");
}else{
echo "<p>이미 사용한 경매번호 입니다.<br>";
