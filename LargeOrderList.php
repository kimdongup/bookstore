...
<td width="710"> &nbsp;<B>대량구매(총등록수 : <?php echo number_format($numrows); ?>건)</B>
</td>
</tr>
...
$num=1;
$numresult=mysqli_query($link,"select * from largepurchase order by id desc limit $fromnum,$limit50");
...
<br>정가 : <?php echo number_format($row['fixprice']); ?>
...
<br><font color="blue">대량구매 할인가(1EA) : <?php echo number_format($row['dcprice']); ?>원</font>
<br>현재 구매 수 : <?php echo number_format($row['orderquantity']); ?>개
<br>최소 구매 수 : <?php echo number_format($row['smallquantity']); ?>개
...
<td>
<form name="Form1" method="post" action=largeOrderWrite.php?id=<?php echo $row['id']?>&goodsid=<?php echo $rowbody['id']?>> 
<input type="text" name="orderquantity" value="" size="12">EA
<span style=font-size:5pt;>&nbsp;</span><br> 
<input type="submit" name=submit value="[대량구매 신청하기]">
</form>
