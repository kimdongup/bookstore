...
$src_im = ImageCreateFromJPEG("$imagename");
$width=imagesx($src_im);
$height=imagesy($src_im);
$dst_im = ImageCreatetruecolor(100, 130);

$white = ImageColorAllocate($dst_im, 0, 0, 0);

ImageCopyResampled($dst_im, $src_im, 0, 0, 0, 0, 100, 130, $width, $height);
ImageJPEG($dst_im,$s_img);
ImageDestroy($src_im);
ImageDestroy($dst_im);
?>