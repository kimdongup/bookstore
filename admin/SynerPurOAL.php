...
if($mod=="synerorderdel"){ // 주문취소

$queryu=mysqli_query($link,"select * from synergicpurchase where id='$updateid'");
$rowu=mysqli_fetch_array($queryu);
$uquantity=$rowu['orderquantity']-1;
$uprice=$rowu['price']-$rowu['dcprice'];

$update2 = "update synergicpurchase set orderquantity='$uquantity' where id='$updateid' ";
$result2=mysqli_query($link,$update2);

$update3 = "update synergicpurchase set price='$uprice' where id='$updateid' ";
$result3=mysqli_query($link,$update3);

$del = "delete from synerorder where id='$delid' ";
$result=mysqli_query($link,$del);
}
$numresults=mysqli_query($link,"select id from synerorder");
...
<td width="710"> &nbsp;<B>공동구매 주문내역(총등록수 : <?php echo number_format($numrows); ?>건)</B>
</td>
</tr>
...
<td width="100" height="30" align="center">시작가</td>
<td width="100" height="30" align="center">현재가</td>
<td width="100" height="30" align="center">구매수</td>
<td width="100" height="30" align="center">마감날자</td>
...
$num=1;
$numresult=mysqli_query($link,"select * from synerorder order by id desc limit $fromnum,$limit50");
while ($row=mysqli_fetch_array($numresult)){

if($numrows < 1)
echo "<tr><td colspan=3 height=25>공동구매 주문내역이 없습니다.</td></tr>";

$query2=mysqli_query($link,"select * from synergicpurchase where id='$row[synerid]'");
...
<td width="100" align="center" style="border-bottom-width:1; border-bottom-color:CFCADF; border-bottom-style:dotted;">
<form>
<input type="button" Value="상세보기" onClick="window.open ('SynerOrderMinute.php?orderid=<?php echo $row['id']?>&name=<?php echo $rowbody['name']?>', 'new', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=auto, resizable=no,copyhistory=no,width=800, height=300')">
</td></form>
<td width="100" align="center" style="border-bottom-width:1; border-bottom-color:CFCADF; border-bottom-style:dotted;">
<a href="<?php echo $_SERVER['SCRIPT_NAME']?>?delid=<?php echo $row['id']?>&updateid=<?php echo $row2['id']?>&mod=synerorderdel">주문취소</a>
</td>
</tr> 
<?php
}
