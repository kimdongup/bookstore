...
if(!isset($_REQUEST['cash'])){$cash ="";} else {$cash = $_REQUEST['cash'];}
if(!isset($_REQUEST['point'])){$point ="";} else {$point = $_REQUEST['point'];}
if(!isset($_REQUEST['visit'])){$visit ="";} else {$visit = $_REQUEST['visit'];}
if(!isset($_REQUEST['grade'])){$grade ="";} else {$grade = $_REQUEST['grade'];}
...
if($mod == "modify_yes"){
    ...
    if($imgup1_size > 10 && $imgup1_size < 300000){ 
        ...
        $widthsize1=@GetImageSize($filewidth1); //이미지의 넓이
        $fwidth1=640;  //화면에 보여질 최대 넓이를 정한다.
		$fheight1=480;  //화면에 보여질 최대 높이를 정한다.

        //실제 이미지 사이즈가 지정된 사이즈보다 작거나 같으면 사이즈를 그대로 유지한다.
   ...
   if(!move_uploaded_file($_FILES['imgup1']['tmp_name'],$dest)) {
    ...
    if($emailsj  == 1){
        ...
        if(mysqli_num_rows(mysqli_query($link,$query3)) > 0) {
            ...
            $update2 = "update member set  email='$email' where id='$id' ";
            $result2=mysqli_query($link,$update2);
        }   
    }

    $update = "update member set pass1='$pass1',pass2='$pass2',cash='$cash',cashday='$timeno',point='$point',visit='$visit',grade='$grade',tel1='$tel1',tel2='$tel2',tel3='$tel3',htel1='$htel1',htel2='$htel2',htel3='$htel3',post1='$post1',post2='$post2',address='$address',post_num='$post_num',mailing='$mailing',hobby1='$hobby1',hobby2='$hobby2',hobby3='$hobby3',hobby4='$hobby4',hobby5='$hobby5',hobby6='$hobby6',hobby7='$hobby7',job='$job',url='$url',sms='$sms',ip='$_SERVER[REMOTE_ADDR]',avatar='$avatar',myinfo='$myinfo' where id='$id' ";
    $result=mysqli_query($link,$update);
