...
if($my_admin_id && $my_admin_pwd){
	if($mod=='auctionpurdel'){ 

	if($mod2=='delok'){
		$querydel=mysqli_query($link,"delete from auctionpurchase where id=$id");
		returnurl('auctionPurList.php');
	}else{
	?>
		<p><b><?php echo $delname; ?></b></p>
		이 상품을 정말로 삭제하시겠습니까?<p>
		<a href=<?php echo $_SERVER['SCRIPT_NAME']?>?id=<?php echo $id?>&mod=auctionpurdel&mod2=delok>[예]</a>
	<?php
	}
	}else{
